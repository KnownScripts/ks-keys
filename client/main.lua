...
function LockpickDoor(vehicle, isAdvanced)
    ...
    usingAdvanced = isAdvanced
    local time = 0.19
    AttemptPoliceAlert("Auto Muukimine / Varastamine")
    Wait(3000)
    local success = lib.skillCheck({{areaSize = 87, speedMultiplier = time},'easy',{areaSize = 87, speedMultiplier = time},'easy',{areaSize = 87, speedMultiplier = time}, 'easy',{areaSize = 87, speedMultiplier = time},'easy'}, {'e', 'e', 'e', 'e'})
    AttemptPoliceAlert("Auto Muukimine / Varastamine")
    if success then
        ...
        ClearPedTasks(ped)
        StopAnimation()
        TriggerEvent('ks-keys:RemoveLockpick')    
            
    else
        AttemptPoliceAlert("Auto Muukimine / Varastamine")
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
        QBCore.Functions.Notify("Sa ebaõnnestusid", "error")
        StopAnimation()
        TriggerEvent('ks-keys:RemoveLockpick')
    end
...
function Hotwire(vehicle, plate)
    ...
    StartAnimation()
    local success = lib.skillCheck({{areaSize = 25,speedMultiplier = time},'easy',{areaSize = 25,speedMultiplier = time},'easy',{areaSize = 25,speedMultiplier = time},'easy'})
    --local success = lib.skillCheck({{areaSize = 50, speedMultiplier = time},'easy',{areaSize = 50, speedMultiplier = time},'easy',{areaSize = 50, speedMultiplier = time},'easy'})
    ...
    if success then
        ...
        AttemptPoliceAlert("Auto Muukimine / Varastamine")
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
        QBCore.Functions.Notify("Sul ei õnnestunud autot muukida !", "error")
    end
